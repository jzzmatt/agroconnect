...
export function ProductsDashboardClient({ initialProducts }: ProductsDashboardClientProps) {
  const { dict } = useI18n();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok || !payload.success) {
        setDeleteError(
          localizeError(dict, payload.code, dict.products.deleteFailed)
        );
        return;
      }
      ...
      setDeleteError(dict.products.deleteFailed);
    } finally {
      setIsUpdating(null);
    }
  };
    ...
    <div className="space-y-6">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50">
          <div className="bg-surface-card border border-border rounded-3xl p-6 max-w-md w-full shadow-xl space-y-4">
            <h3 className="text-lg font-black text-foreground">{dict.products.deleteConfirmTitle}</h3>
            <p className="text-sm text-muted-foreground">
              {dict.products.deleteConfirmBody.replace("{title}", deleteTarget.title)}
            </p>
            {deleteError && (
