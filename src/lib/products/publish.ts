...
export async function ensureSellerProfile(profile: UserProfileWithRoles) {
  ...
  const client = await writableClient();
  if (!isUuid(profileId)) {
    const { data: existingProfile } = await withSupabaseRetry("profiles.select", () =>
    ...
    if (existingProfile?.id && isUuid(existingProfile.id)) {
      ...
      const { data: createdProfile, error: profileError } = await withSupabaseRetry(
        "profiles.upsert",
        () =>
          (client.from("profiles") as any)
            .upsert(
  ...
  const { data: existing } = await withSupabaseRetry("provider_profiles.select", () =>
    (client.from("provider_profiles") as any)
      .select("*")
  ...
  const slugBase = (profile.display_name || "vendedor")
    ...
    .replace(/^-|-$/g, "")
    .slice(0, 40) || "vendedor";
  const { data: created, error } = await withSupabaseRetry("provider_profiles.insert", () =>
    (client.from("provider_profiles") as any)
      .insert({
...
export async function insertProductRow(params: {
  ...
  if (!categoryId) {
    const { data: categoryRow } = await withSupabaseRetry("categories.select", () =>
  ...
  const { data, error } = await withSupabaseRetry("products.insert", () =>
    (client.from("products") as any)
      .insert({
